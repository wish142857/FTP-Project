...
void echo(int sd) {
    ...
    while (1) {
      ...
      if (n < 0) {
        ...
        /* Add Sequence Number, and send it back */
        sprintf(output, "%d %s", seqNum++, bufin);
        uppercase(output);
        sendto(sd, output, strlen(output), 0, (struct sockaddr *)&remote, len);
        memset(bufin, 0, sizeof(output));
      }
    }
}
